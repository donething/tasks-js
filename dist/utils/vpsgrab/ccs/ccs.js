...
// 数据文件的路径
const dbPath = comm_1.Root + "/ccs_order.json";
/**
 * 登录
 * 登录失败时，会直接抛出错误
 */
const login = async (user) => {
    ...
    const loginHtmlResp = await http_1.mAxios.get(`${addr}/index.php?rp=/login`, { headers });
    ...
    const resp = await http_1.mAxios.post(`${addr}/index.php?rp=/login`, data, { headers });
...
const order = async (productUrl, user) => {
    ...
    const checkoutResp = await http_1.mAxios.post(`${addr}/cart.php?a=checkout`, checkoutData, { headers });
...
const startOrder = async (users) => {
    // 读取数据
    const fData = (0, file_1.readJSON)(dbPath, { hadOrder: [], hadNotifyUnavailable: false });
    ...
    for (const [i, result] of results.entries()) {
        ...
        if (available.length === 0) {
            ...
            await (0, tgpush_1.pushTGMsg)("账号不足", "已没有还未订购VPS的账号", TAG);
