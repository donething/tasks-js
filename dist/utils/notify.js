...
const bulletpush_1 = require("./push/bulletpush");
...
const notifyTopics = async (taskInfo) => {
    // 读取已提示的帖子列表（ID 列表）
    const data = (0, file_1.readJSON)(taskInfo.filepath, { topics: [] });
    // 临时保存已发送的帖子
    const hadSend = [];
    // 异步执行所有任务
    ...
    for (const [i, result] of results.entries()) {
        if (result.status === "rejected") {
            ...
            (0, tgpush_1.pushTGMsg)("执行失败", err.message, taskInfo.tag + " " + taskInfo.topicTaskInfos[i].tag);
        }
    }
    if (hadSend.length === 0) {
