...
import {envTip} from "./utils/spider/base/comm"
 ...
 * @param rgba2 遮罩色
 */
const combineRgba = (rgba1: number[], rgba2: number[]): number[] => {
 ...
 * 判断两个颜色是否相似
 */
const tolerance = (rgba1: number[], rgba2: number[], t: number): boolean => {
...
// 获取验证位置
const getVerifyPosition = (base64: string, actualWidth: number): Promise<number> => {
...
// 登录
const login = async (page: Page, username: string, password: string): Promise<boolean> => {
  ...
  await simulateHumanMove(page, {x, y}, {x: x + distance, y: y + 50})
  await page.mouse.up()
...
const startJDCookie = async () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    try {
      ...
      console.log("登录失败：", e)
      if (typeError(e).message.includes("账号名与密码不匹配")) {
        break
      }
    }
  }
